  ...
  return `
{
  ...
  "files": [
    "src/_library/${groupDir}${name}/${indexFile}"
  ]
}
  `;
};
...
const createWebpackConfig = ({name, groupDir, indexFile, libExternals}) => {
  const copyPlugin = (() => {
    const from = 'src/_library/' + groupDir + name;
...
const include = file => {
  return file.indexOf(path.resolve(__dirname, 'src/_library/${groupDir}${name}')) === 0;
};
...
module.exports = () => new Promise(resolve => {
  ...
  const config = {
    ...
    entry: () => './src/_library/${groupDir}${name}/${indexFile}',
...
console.log(Object.keys(libs.entry).reduce(($, name) => {
  ...
  const indexFile = fs.existsSync('src/_library/' + groupDir + name + '/index.tsx')
    ? 'index.tsx'
    : 'index.ts';
